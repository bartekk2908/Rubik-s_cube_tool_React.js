...
export function TimerTab({ holdingSpaceTime, determineVisibilityFunc, scrambleLength, results, algorithmsData, trainingStateDict, outerPopupOpened, settings }) {
    ...
    function giveReversedSequence(sequence) {
        function giveReversedMove(move) {
            return (move[1] === "2" ? (move) : (move[1] === "'" ? (move[0]) : (move[0] + "'")));
        }
        ...
        function replaceWideMoves(array) {
            ...
            for(let i=n-1; i>=0; i--) {
                ...
                if ((Array.from(changes.keys())).includes(currentLetter[0])) {
                    ...
                    if (currentLetter[1] === "'") {
                        forReplace = [giveReversedMove(forReplace[0]), giveReversedMove(forReplace[1])];
                    } else if (currentLetter[1] === "2") {
                        forReplace = [forReplace[0][0] + "2", forReplace[1][0] + "2"];
                    }
        ...
        const moves = removeRotations(replaceWideMoves(replaceWideMoves(sequence.split(" "))));
